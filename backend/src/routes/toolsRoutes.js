...
const { addTool } = require('../controllers/ToolsController');
...
router.get('/:id', (req, res) => {
  const query = 'SELECT * FROM tools WHERE T_id = ?';
  const { id } = req.params;
...
// Add a new tool
router.post('/', (req, res) => {
  const { T_name, T_type, T_PowerSource } = req.body;

  // Validate input
  if (!T_name || !T_type || !T_PowerSource) {
    return res.status(400).json({ error: 'All fields are required' });
  }

  const query = 'INSERT INTO tools (T_name, T_type, T_PowerSource) VALUES (?, ?, ?)';
  const values = [T_name, T_type, T_PowerSource];
